...
*/
// This is the new, unified app.js file.
// It includes the game, calendar, results charts, AI insights, and chatbot.
...
const JOURNAL_KEY = 'irt:journal';
const SESS_KEY = 'irt:sessions';
const patientData = [ // Sample data for AI analysis
  { session: 1, avgTime: 3.5, errors: 7, range: 4 },
  { session: 2, avgTime: 3.1, errors: 5, range: 5 },
...
function DashboardPage() {
    ...
    <section class="panel">
      <div class="row" style="justify-content:space-between; align-items:center;">
        ...
        <div class="row" style="gap:8px">
          <a class="btn secondary" href="#/calendar">Calendar</a>
          <a class="btn secondary" href="#/results">Results</a>
          <a class="btn secondary" href="#/ai-insights">AI Insights</a>
        </div>
      </div>
      <div class="card" style="margin-top:12px">
        <div class="row" style="justify-content:space-between; align-items:center;">
          <div class="row" style="gap:8px;"><h2 style="margin:0">Game</h2><span class="badge">Live</span></div>
          <button class="btn secondary" id="reset-game">Reset</button>
        </div>
        <div class="canvas-wrap" style="margin-top:8px">
  ...
  window.onGameSessionComplete = (payload) => {
    const s = { date: payload?.date || new Date().toISOString(), level: Number(payload?.level || 1), metrics: { avgSpeedSec: Number(payload?.avgSpeedSec || 0), errors: Number(payload?.errors || 0), romZones: Number(payload?.romZones || 0) } };
    saveSession(s);
    location.hash = '#/results';
  };

  document.getElementById('reset-game')?.addEventListener('click', () => { if (typeof window.initInjuryGame === 'function') window.initInjuryGame(); });
}
...
function ResultsPage() {
    const j = loadJournal(); const days = Object.keys(j).sort(); const latest = days.length ? j[days.at(-1)] : null; const sessions = loadSessions(30);
    app.innerHTML = `<section class="panel"><div class="row" style="justify-content:space-between;"><div><h1>Results</h1><p>Summary of your check-ins and recent game sessions.</p></div><a class="btn secondary" href="#/dashboard">Dashboard</a></div><div class="grid grid-3" style="margin-top:12px"><div class="card"><h2>Latest Check-in</h2>${latest ? renderEntry(latest) : '<p>No entries yet.</p>'}</div><div class="card"><h2>Average Mood</h2><p style="font-size:40px; margin: 12px 0;">${avg(Object.values(j).map(e => e.mood || 0)).toFixed(1)}</p><p class="badge">${Object.keys(j).length} days logged</p></div><div class="card"><h2>Wellbeing Breakdown</h2>${renderWellbeingBars(Object.values(j))}</div></div><div class="card" style="margin-top:16px"><h2>Recent Game Sessions (last 10)</h2>${sessions.length ? `<table class="table"><thead><tr><th>Date</th><th>Avg Speed (s)</th><th>Errors</th><th>ROM</th></tr></thead><tbody>${sessions.slice(-10).reverse().map(s => `<tr><td>${(s.date||'').slice(0,10)}</td><td>${(s.metrics?.avgSpeedSec||0).toFixed(2)}</td><td>${s.metrics?.errors||0}</td><td>${s.metrics?.romZones||0}</td></tr>`).join('')}</tbody></table>` : '<p>No sessions yet.</p>'}</div><div class="grid grid-2" style="margin-top:16px"><div class="card"><h2>Error Rate Trend</h2><canvas id="errorsChart"></canvas></div><div class="card"><h2>Average Speed Trend</h2><canvas id="speedChart"></canvas></div></div></section>`;
    if (sessions.length > 0) {
        const labels = sessions.map(s => (s.date || '').slice(5, 10)); const errorData = sessions.map(s => s.metrics?.errors || 0); const speedData = sessions.map(s => s.metrics?.avgSpeedSec || 0);
        const errorsCtx = document.getElementById('errorsChart').getContext('2d');
        new Chart(errorsCtx, { type: 'line', data: { labels: labels, datasets: [{ label: 'Errors per Session', data: errorData, borderColor: '#f56565', backgroundColor: 'rgba(245, 101, 101, 0.2)', fill: true, tension: 0.3 }] } });
        const speedCtx = document.getElementById('speedChart').getContext('2d');
        new Chart(speedCtx, { type: 'line', data: { labels: labels, datasets: [{ label: 'Average Speed (s)', data: speedData, borderColor: '#4299e1', backgroundColor: 'rgba(66, 153, 225, 0.2)', fill: true, tension: 0.3 }] } });
    }
}
...
function AI_InsightsPage() {
  app.innerHTML = `
    <section class="panel">
      <h1>AI Insights: Correlations in Patient Activity</h1>
      <div class="row" style="gap:8px; justify-content:center; margin-bottom:16px;">
        <a class="btn secondary" href="#/dashboard">Back to Dashboard</a>
      </div>
      <div class="card">
        <div class="button-row" style="display:flex; gap:8px; justify-content:center;">
          <button id="analyze-btn" class="btn">Analyze Data for Correlations</button>
          <button id="outliers-btn" class="btn">Analyze Data for Outliers</button>
        </div>
        <div id="ai-analysis-output" style="margin-top:16px; min-height:50px; padding:8px; background: #0b1530; border-radius:8px;">Click a button to begin analysis.</div>
      </div>
    </section>
  `;
  ...
  document.getElementById("analyze-btn").addEventListener("click", async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/analyze-data`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: patientData })
      });
      if (!response.ok) throw new Error('Server response was not ok');
      const result = await response.json();
      outputDiv.textContent = result.analysis;
...
// --- Initializers that run once the page is loaded ---
function initFloatingChatbot() {
  const chatToggleBtn = document.getElementById('chat-toggle-btn');
  const chatbotContainer = document.getElementById('floating-chatbot-container');
  if (!chatToggleBtn || !chatbotContainer) return;
  ...
  chatToggleBtn.addEventListener('click', () => { chatbotContainer.classList.toggle('is-visible'); });

  function addMessageToChatbox(text, sender) { /* ... same as team's code ... */ }
  
  const handleSend = async () => {
    const userMessage = chatInput.value.trim();
    if (userMessage === "") return;
    if (!hasGreeted) { chatbox.innerHTML = ""; hasGreeted = true; }
    addMessageToChatbox(userMessage, "user");
    chatInput.value = "";
    try {
      addMessageToChatbox("Typing...", "system");
      const response = await fetch(`${API_BASE_URL}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: userMessage, data: patientData })
      });
      const result = await response.json();
      chatbox.removeChild(chatbox.lastChild);
      addMessageToChatbox(result.response, "system");
  ...
  chatInput.addEventListener("keypress", (event) => { if (event.key === "Enter") { handleSend(); } });
}
...
function setupThemeToggle() {
    ...
    toggleButton.addEventListener('click', () => {
        ...
        document.body.classList.toggle('dark-theme', isDark);
        document.body.classList.toggle('light-theme', !isDark);
        themeIcon.src = isDark ? "./img/mode.png" : "./img/dark-mode.png";
    });
}
...
window.addEventListener('load', () => {
  router();
  // Check for chatbot elements before initializing

...
function checkLevelEnd() {
    if (orbs.every(o => o.fed)) {
        ...
        // --- ADD THIS LINE ---
        // This makes the button to view the graphs appear.
...
function showStats() {
    ...
    statsDiv.innerHTML = `
        ...
        Average Speed: <b>${speedAvg}s</b> | Misses/Errors: <b>${acc}</b>
    `;

    // --- ADD THIS LOGIC TO SAVE THE DATA ---

    // 1. Get existing history from localStorage or create a new empty array
    const levelHistory = JSON.parse(localStorage.getItem('levelHistory')) || [];

    // 2. Add the new result to the array
    levelHistory.push({
        level: level,
        misses: acc,
